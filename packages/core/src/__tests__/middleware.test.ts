...
describe("composeModel", () => {
  it("runs middleware outside-in (onion order)", async () => {
    ...
    const run = composeModel([a, b], async () => { order.push("base"); return { finishReason: "stop" as const }; });
...
describe("policyMiddleware", () => {
  ...
  it("allows non-sensitive tools by default", async () => {
    ...
    const tool = defineTool({ name: "read", description: "", parameters: z.object({}), execute: () => Promise.resolve("ok") });
    expect(await run({ tool, args: {}, ctx }, () => Promise.resolve("ok"))).toBe("ok");
  });

  it("blocks sensitive tools when no approver is set", async () => {
    const run = policyMiddleware([]);
    const tool = defineTool({ name: "send", description: "", sensitive: true, parameters: z.object({}), execute: () => Promise.resolve("ok") });
    await expect(run({ tool, args: {}, ctx }, () => Promise.resolve("ok"))).rejects.toThrow(/approval/i);
  });
});
